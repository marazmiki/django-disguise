...
@pytest.fixture
def super_user():
    return get_user_model().objects.create_superuser(username='root',
                                                     password='root',
                                                     email='root@example.com')

@pytest.fixture
def regular_user():
    return get_user_model().objects.create_user(username='user',
                                                password='user',
...
def test_if_user_can_see_disquise_form(django_client, url, name, should, request):
    django_client.logout()
    if name:
        django_client.force_login(user=request.getfixturevalue(name))
    assert '<form ' in django_client.get(url).content is should
